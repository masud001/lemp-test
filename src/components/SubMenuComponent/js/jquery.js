...
if (typeof window.jQuery == "undefined") {
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        attr: function(key, value, type) {
            ...
            if (key.constructor == String)
                if (value == undefined) return this.length && jQuery[type || "attr"](this[0], key) || undefined;
                else {
                    obj = {};
                    obj[key] = value
    ...
    jQuery.extend({
        ...
        merge: function(first, second) {
            ...
            for (var i = 0, sl = second.length; i < sl; i++)
                if (jQuery.inArray(second[i], r) == -1) first.push(second[i]);
            return first
        },
        grep: function(elems, fn, inv) {
            ...
            for (var i = 0, el = elems.length; i < el; i++)
                if (!inv && fn(elems[i], i) || inv && !fn(elems[i], i)) result.push(elems[i]);
            return result
        },
        map: function(elems, fn) {
            ...
            check: for (var i = 1, rl = result.length; i < rl; i++) {
                for (var j = 0; j < i; j++)
                    if (result[i] == r[j]) continue check;
                r.push(result[i])
            }
            return r
        }
                ...
                if (m) {
                    jQuery.each(ret, function() {
                        for (var c = this.firstChild; c; c = c.nextSibling)
                            if (c.nodeType == 1 && (jQuery.nodeName(c, m[1]) || m[1] == "*")) r.push(c)
                    });
                    ret = r;
                    t = t.replace(re, "");
                ...
                if (t && !foundToken) {
                    if (!t.indexOf(",")) {
                        ...
                        if (m[1] == "#" && ret[ret.length - 1].getElementById) {
                            ...
                            if (m[1] == "." && ret.length == 1) r = jQuery.grep(r, function(e) {
    ...
    jQuery.event = {
            ...
            if (element.$events) {
                ...
                if (type && element.$events[type])
                    if (handler) delete element.$events[type][handler.guid];
                        ...
                        for (i in element.$events[type]) delete element.$events[type][i];
                else
                    for (j in element.$events) this.remove(element, j);
        ...
        trigger: function(type, data, element) {
            ...
            if (!element) jQuery.each(this.global[type] || [], function() {
                jQuery.event.trigger(type, data, this)
            });
            else {
                var handler = element["on" + type],
                    val, fn = jQuery.isFunction(element[type]);
                if (handler) {
    ...
    if (jQuery.browser.msie) jQuery(window).one("unload", function() {
        ...
        for (var type in global) {
            var els = global[type],
                i = els.length;
            if (i && type != 'unload')
                do jQuery.event.remove(els[i - 1], type); while (--i)
        }
    });
    jQuery.fn.extend({
            ...
            z.step = function(firstNum, lastNum) {
                ...
                if (t > options.duration + z.startTime) {
                    ...
                    for (var i in elem.curAnim)
                        if (elem.curAnim[i] !== true) done = false;
                    if (done) {
                        y.overflow = "";
